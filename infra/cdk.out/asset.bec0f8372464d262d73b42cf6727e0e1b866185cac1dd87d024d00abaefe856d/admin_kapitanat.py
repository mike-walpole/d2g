...
def handle_cargo_types(http_method: str, query_params: Dict, body: Dict) -> Dict[str, Any]:
    ...
    if http_method == 'GET':
        ...
        new_version = save_schema_version(
            change_type="cargo_type_added",
            changed_component="cargoTypes",
            change_details=f"Added cargo type: {new_cargo_type['name']['en']} (ID: {new_cargo_type['id']})"
        )
        ...
        # Create new schema version
        new_version = save_schema_version(
            change_type="cargo_type_updated",
            changed_component="cargoTypes",
            change_details=f"Updated cargo type: {cargo_to_update['name']['en'] if isinstance(cargo_to_update.get('name'), dict) else 'Unknown'} (ID: {cargo_to_update['id']})"
        )
        ...
        # Create new schema version
        new_version = save_schema_version(
            change_type="cargo_type_deleted",
            changed_component="cargoTypes",
            change_details=f"Deleted cargo type ID: {cargo_id}"
        )
    ...
    try:
        ...
        if version == 'current':
            ...
            try:
                ...
                schema_data = response['Item']['schema']
                download_data = {
                    'downloadedAt': datetime.utcnow().isoformat(),
        ...
        return {
            ...
            'body': json.dumps(download_data, indent=2, ensure_ascii=False)
        }
