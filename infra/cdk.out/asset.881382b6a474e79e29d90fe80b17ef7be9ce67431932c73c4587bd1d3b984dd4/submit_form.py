...
def generate_sequential_id() -> str:
    """
    Generate a sequential 8-digit ID for submissions starting from 00000001
    Uses DynamoDB atomic counter to ensure uniqueness
    """
    try:
        schemas_table = dynamodb.Table(os.environ['SCHEMAS_TABLE'])
        
        # Use atomic update to increment the counter, initializing to 0 if it doesn't exist
        response = schemas_table.update_item(
            Key={
                'formId': 'config',
                'version': 'submission-counter'
            },
            UpdateExpression='SET submission_counter = if_not_exists(submission_counter, :start) + :inc',
        ...
        counter = response['Attributes']['submission_counter']
        
        # Format as 8-digit zero-padded string
        submission_id = f"{counter:08d}"
        
        print(f"Generated sequential submission ID: {submission_id}")
        return submission_id
        
    except Exception as e:
        print(f"Error generating sequential ID: {e}")
        # Fallback to timestamp-based ID if counter fails
        fallback_id = f"{int(datetime.utcnow().timestamp()) % 100000000:08d}"
        print(f"Using fallback timestamp-based ID: {fallback_id}")
        return fallback_id
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Generate sequential 8-digit ID for the submission
        submission_id = generate_sequential_id()
        ...
        try:
            ...
            # Create subject line with format: "Dock2Gdansk:00000001@wersja:1.0.0"
