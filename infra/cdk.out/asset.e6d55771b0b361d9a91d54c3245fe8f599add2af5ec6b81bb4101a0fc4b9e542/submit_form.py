...
def get_admin_email_addresses():
    """
    Get email addresses of all enabled admin users from Cognito User Pool
    """
    try:
        user_pool_id = os.environ.get('USER_POOL_ID')
        if not user_pool_id:
            print("Warning: USER_POOL_ID not found, using fallback recipients")
            return ['walpole.mike@gmail.com', 'Oliwia.Walter@portgdansk.pl']
        ...
        for user in response['Users']:
            ...
            if user.get('Enabled', False) and user.get('UserStatus') == 'CONFIRMED':
                user_attributes = {attr['Name']: attr['Value'] for attr in user.get('Attributes', [])}
                email = user_attributes.get('email')
                
                if email:
                    # Check if user has admin privileges (optional - could check groups)
                    try:
                        user_groups = cognito.admin_list_groups_for_user(
                            UserPoolId=user_pool_id,
                            Username=user['Username']
                        )
                        # If user has any groups (admin groups), include them
                        # For now, include all confirmed users as potential recipients
                        admin_emails.append(email)
                    except Exception as e:
                        print(f"Warning: Could not get groups for user {user['Username']}: {e}")
                        # Include the user anyway if they're confirmed
                        admin_emails.append(email)
        
        # Fallback to hardcoded recipients if no admin emails found
        if not admin_emails:
            print("No admin emails found in Cognito, using fallback recipients")
            admin_emails = ['walpole.mike@gmail.com', 'Oliwia.Walter@portgdansk.pl']
        
        print(f"Admin email recipients: {admin_emails}")
        return admin_emails
        
    except Exception as e:
        print(f"Error getting admin emails from Cognito: {e}")
