...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if config_type in ['all', 'referral-sources']:
            # Get referral sources with translations
            response = schemas_table.get_item(
                Key={
                    'formId': 'config',
            ...
            if 'Item' in response:
                referral_sources = response['Item']['schema']['referralSources']
                # Transform referral sources for frontend consumption
                transformed_sources = []
                for source in referral_sources:
                    if source.get('active', True):
                        # Handle both old format (string name) and new format (object with translations)
                        if isinstance(source['name'], dict):
                            name = source['name'].get(language, source['name'].get('en', 'Unknown'))
                        else:
                            name = source['name']  # Fallback for old format
                ...
                result['referralSources'] = transformed_sources
            else:
                result['referralSources'] = []
