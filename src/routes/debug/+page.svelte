...
<div style="max-width: 800px; margin: 100px auto 50px; padding: 20px; font-family: monospace;">
	...
	{:else}
		<div style="display: grid; gap: 20px;">
			<!-- Server-side Results -->
			<div style="padding: 20px; background: #f0f8ff; border-radius: 8px; border-left: 4px solid #0043ce;">
				<h3 style="margin: 0 0 15px 0; color: #0043ce;">🖥️ Server-side Detection (Vercel Headers)</h3>
				<pre style="background: #ffffff; padding: 15px; border-radius: 4px; overflow-x: auto;">{JSON.stringify(geolocationResults.serverSide, null, 2)}</pre>
			</div>
			...
			<div style="padding: 20px; background: #fef3c7; border-radius: 8px; border-left: 4px solid #f59e0b;">
				...
				<div style="background: #ffffff; padding: 15px; border-radius: 4px;">
					...
					<p><strong>Translation Test:</strong> {t('site_title', 'Dock2Gdansk')} - {t('site_tagline', 'Professional Cargo Transportation')}</p>
				</div>
			</div>
			
			<!-- User Preferences -->
			<div style="padding: 20px; background: #f3e8ff; border-radius: 8px; border-left: 4px solid #8b5cf6;">
				<h3 style="margin: 0 0 15px 0; color: #7c3aed;">👤 User Preferences (localStorage)</h3>
				<pre style="background: #ffffff; padding: 15px; border-radius: 4px; overflow-x: auto; font-size: 12px;">{JSON.stringify(geolocationResults.userPreferences, null, 2)}</pre>
			</div>
			...
			<div style="padding: 20px; background: #fee2e2; border-radius: 8px; border-left: 4px solid #ef4444;">
				...
				<div style="display: flex; gap: 10px; flex-wrap: wrap;">
					<button onclick={clearPreferences} style="padding: 10px 20px; background: #ef4444; color: white; border: none; border-radius: 4px; cursor: pointer;">
						Clear Preferences
					</button>
					<button onclick={testGeolocation} style="padding: 10px 20px; background: #0043ce; color: white; border: none; border-radius: 4px; cursor: pointer;">
						Refresh & Test
					</button>
					<a href="/" style="padding: 10px 20px; background: #22c55e; color: white; text-decoration: none; border-radius: 4px; display: inline-block;">
						Back to Home
					</a>
				</div>
			</div>
			...
			<div style="padding: 20px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
				...
				<ol style="margin: 0; padding-left: 20px; line-height: 1.6;">
					<li><strong>Server-side (Vercel):</strong> Uses <code>x-vercel-ip-country</code> header from Vercel Edge Network</li>
					<li><strong>Client-side fallback:</strong> Uses free IP geolocation APIs (ipapi.co, ip-api.com, etc.)</li>
					<li><strong>Language mapping:</strong> CN/HK/MO/TW/SG → Chinese (zh), all others → English (en)</li>
					<li><strong>User preferences:</strong> Manual language selection overrides geolocation</li>
					<li><strong>localStorage keys:</strong>
						<ul style="margin: 5px 0;">
							<li><code>preferred-language</code>: User's saved language choice</li>
							<li><code>language-override</code>: Set to 'true' when user manually selects language</li>
